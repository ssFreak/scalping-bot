...
class EMARsiTrendScalper(BaseStrategy):
    """
    Versiune Universală (Live + Backtest Portofoliu).
    Detectează automat modul de rulare.
    """
    ...
    def run_once(self, current_bar=None):
        """
        Rulează logica. Primește 'current_bar' în backtest, 
        sau cere date dacă 'current_bar' este None (modul live).
        """
        try:
            entry_price = 0.0
            current_m5_atr = 0.0
            ...
            # Detectăm automat modul de rulare
            ...
            if not is_backtest_run:
                ...
                if df_m5 is None or len(df_m5) < 50: return
                ...
                if self.broker.has_open_position(self.symbol): return
            ...
            if current_m5_atr <= 0.0: 
                if not is_backtest_run: self.logger.log(f"{log_prefix} ⚠️ [FILTER] ATR M5 este 0 sau negativ. Aștept...")
                return
            ...
            if not is_backtest_run:
                ...
                lot = self.broker.calculate_lot_size(
                    self.symbol, 
                    sl_distance_price / pip
                )
            
            if lot <= 0:
                if not is_backtest_run: self.logger.log(f"{log_prefix} ⚠️ [FILTER] Lotul calculat este 0. Aștept...")
                return

            ml_features = {
                'H1_trend_up': bool(trend_up),
                'M5_rsi': round(current_m5_rsi, 2),
                'M5_atr': round(current_m5_atr, digits)
            }

            if trend_up and current_m5_rsi < self.rsi_oversold:
                sl = round(entry_price - sl_distance_price, digits)
                tp = round(entry_price + tp_distance_price, digits)
                if not is_backtest_run: self.logger.log(f"🚀 Deschidere BUY {self.symbol} @ {entry_price} (RSI: {current_m5_rsi:.2f})")
                self.broker.open_market_order(self.symbol, 0, lot, sl, tp, self.magic_number, "EMA_RSI_BUY", ml_features=ml_features)
            
            elif not trend_up and current_m5_rsi > self.rsi_overbought:
                sl = round(entry_price + sl_distance_price, digits)
                tp = round(entry_price - tp_distance_price, digits)
                if not is_backtest_run: self.logger.log(f"🔻 Deschidere SELL {self.symbol} @ {entry_price} (RSI: {current_m5_rsi:.2f})")
